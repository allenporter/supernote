...
- **🖥️ Sleek Web UI**: Browse notes, manage tasks, and export iCalendar (`.ics`) task feeds for Apple Calendar, Google Calendar, and Outlook.
...
- **iCalendar Feed Export (.ics)**: Preview, copy, and download task feeds in RFC 5545 VTODO `.ics` format directly from the Web UI or via `/api/schedule/feed.ics`.
